...
def read_url_temp() -> List[str]:
    ...
    with open(TEMP_URL_CSV, 'r', encoding="utf-8") as f:
        reader = csv.reader(f)
...
def save_url_temp(urls: List[str]) -> None:
    with open(TEMP_URL_CSV, 'w', encoding="utf-8") as f:
        writer = csv.writer(f)
        for url in urls: 
            writer.writerow([url])
...
def save_news(config:Config, news: News):
    ...
    if not os.path.isfile(filepath):
        with open(filepath, 'w', encoding="utf-8") as header_only:
...
def read_temp_progress() -> Progress:
    if os.path.isfile(TEMP_PROGRESS):
        with open (TEMP_PROGRESS, 'r', encoding="utf-8") as f:
